 ...
 */
export let createEvento = async (evento, id_calendario) => {
    try {
        // Instancia um novo objeto Event com os dados fornecidos.
        let newEvent = new Event(evento.titulo, evento.descricao, evento.data, evento.hora_inicio, evento.hora_fim, evento.id_candidato);
        if (evento.hora_inicio.split(":").length !== 2 ||
            evento.hora_fim.split(":").length !== 2) {
            throw new Error("Formato de hora inválido. Use HH:mm.");
        }
        else if (Number(evento.hora_inicio.split(":")[0]) >
            Number(evento.hora_fim.split(":")[0])) {
            throw new Error("A hora de início não pode ser maior que a hora de fim.");
        }
        let idCandidateIsValis = await Validation.validateId(evento.id_candidato, "tb_candidato");
