...
export let insertIntoCandidate = async (user) => {
    try {
        ...
        await DB.pool.query(`INSERT INTO tb_candidato (
...
export let insertIntoContratante = async (user) => {
    try {
        ...
        await DB.pool.query(`INSERT INTO tb_empresa (
 ...
 */
export let insertIntoIFBR = async (id, name, data, score, id_tupla) => {
    try {
        console.log(`[insertIntoIFBR] Inserindo domínio IFBR ${name} (${id}) para tupla ${id_tupla}`);
        await DB.pool.query(`INSERT INTO tb_ifbr (id_dominio, nome, data_resposta, score, id_ifbr) VALUES ($1, $2, $3, $4, $5);`, [id, name, data, score, id_tupla]);
 ...
 */
export let updateIfbrCandidato = async (id, id_ifbr) => {
    try {
 ...
 */
export let insertIntoColaborador = async (id, name, email, senha, setor) => {
    try {
        console.log(`[insertIntoColaborador] Inserindo colaborador ${id}`);
        await DB.pool.query(`INSERT INTO tb_colaborador (
 ...
 */
export let insertEmpresaColaborador = async (id_colaborador, id_empresa) => {
    try {
        ...
        const empresa = await DB.pool.query(`SELECT cnpj, razao_social FROM tb_empresa WHERE id = $1`, [id_empresa]);
 ...
 * @param id_empresa ID da empresa.
 */
export let updateColaboradorEmpresa = async (id, id_empresa) => {
    try {
        console.log(`[updateColaboradorEmpresa] Atualizando colaborador ${id} na empresa ${id_empresa}`);
        await DB.pool.query(`UPDATE tb_empresa SET id_colaborador = $1 WHERE id = $2;`, [id, id_empresa]);
 ...
 */
export let selectFromTable = async (table) => {
    try {
 ...
 * @returns Resultado da consulta.
 */
export let selectFromIdWhere = async (table, id) => {
    try {
 ...
 */
export let deleteFromTable = async (table, id) => {
    try {
        console.log(`[deleteFromTable] Atualizando status para false no registro ID ${id} da tabela ${table}`);
        let result = await DB.pool.query(`UPDATE ${table} SET status = $1 WHERE id = $2;`, [false, id]);
 ...
 */
export let updateUserColumn = async (table, id, sets, values) => {
    try {
 ...
 */
export const insertVaga = async (id, data_inicio, data_fim, status_vaga, titulo, descricao, salario, localidade, acess, id_creator) => {
    try {
 ...
 */
export let insertEmpVaga = async (vaga, id_empresa) => {
    try {
 ...
 */
export let getEmpByColab = async (id_colaborador) => {
    try {
 ...
 */
export const insertCandidateVaga = async (id_vaga, id_candidate) => {
    try {
        console.log(`[insertCandidateVaga] Iniciando inserção do candidato ${id_candidate} na vaga ${id_vaga}`);
        ...
        console.log(`[insertCandidateVaga] Candidato ${id_candidate} vinculado à vaga ${id_vaga} com sucesso.`);
        return true;
    }
    catch (error) {
        console.error(`[insertCandidateVaga] ERRO ao vincular candidato ${id_candidate} à vaga ${id_vaga}:`, error);
        return false;
    }
};
/**
 ...
 */
export let validateData = async (value, data, table) => {
    try {
        console.log(`[validateData] Verificando existência do valor '${value}' na coluna '${data}' da tabela '${table}'.`);
        let result = await DB.pool.query(`SELECT ${data} FROM ${table} WHERE ${data} = $1`, [value]);
        console.log(`[validateData] Validação concluída. Registros encontrados: ${result.rows.length}`);
        return result.rows.length;
    }
    catch (error) {
        console.error(`[validateData] ERRO ao validar dados na tabela ${table}, coluna ${data}:`, error);
        return false;
    }
};
/**
 ...
 */
export let insertIntoEventos = async (evento, id_calendario) => {
    try {
        console.log(`[insertIntoEventos] Iniciando inserção do evento ${evento.id} no calendário ${id_calendario}`);
        const { id, titulo, descricao, data, hora_inicio, hora_fim, id_candidato } = evento;
        const query = `
    ...
    `;
        ...
        console.log(`[insertIntoEventos] Evento ${id} inserido com sucesso no calendário ${id_calendario}.`);
        return query;
    }
    catch (error) {
        console.error(`[insertIntoEventos] ERRO ao inserir evento ${evento.id} no calendário ${id_calendario}:`, error);
        throw error;
    }
};
/**
 ...
 */
export let getEventosByCalendario = async (id_calendario) => {
    try {
 ...
 */
export let insertCalendario = async (id_calendar, nome, id_empresa) => {
    try {
 ...
 */
export let selectEmpbyCalendar = async (id_empresa) => {
    try {
