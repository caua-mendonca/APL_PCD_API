...
export let createContratante = async (user) => {
    try {
        ...
        let newContratante = new Contratante(user.nome_fantasia, user.razao_social, user.email, user.confirme_email, user.senha, user.confirme_senha, user.cnpj, user.telefone, user.acessibilidade);
        // Gera ID único
        ...
        // Valida e-mail
        let emailIsValid = newContratante.email === newContratante.confirme_email;
        if (emailIsValid === true) {
            emailIsValid = await validateEmailToDB(newContratante.email, "email", "tb_empresa");
        ...
        // Valida senha
        let passwordIsValid = newContratante.senha === newContratante.confirme_senha;
        if (passwordIsValid) {
        ...
        // Valida CNPJ
        let cnpjIsValid = validateCNPJ(newContratante.cnpj);
        if (cnpjIsValid === true) {
...
export let getUser = async (table) => {
...
export let getUserByID = async (table, id) => {
 ...
 */
export let deleteUser = async (table, id) => {
 ...
 */
export let updateUser = async (table, id, body) => {
    ...
    try {
        ...
        if (keys.includes("cpf")) {
            ...
            const cpfIsValid = validateCpf(values[index]);
        ...
        if (keys.includes("data_nascimento")) {
            ...
            const dateIsValid = validateAge(new Date(values[index]));
