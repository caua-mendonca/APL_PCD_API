...
export let conectServ = (PORT) => {
    ...
    APP.post(Routes.createCanditado, async (req, res) => {
        ...
        try {
            let body = req.body;
            console.log(`[POST ${Routes.createCanditado}] Requisição recebida, corpo:`, body);
            let [status, message] = await controllerCandidate.controllerPostCandadate(body);
    ...
    APP.get(Routes.getCanditado, Middleware.authenticateTokenCand, async (req, res) => {
        console.log(`Requisição recebida em ${Routes.getCanditado}`);
        try {
            console.log(`[GET ${Routes.getCanditado}] Requisição recebida`);
            let [status, message] = await controllerCandidate.controllerGetCandidato();
            res.status(status).send({ message: message });
        }
        catch (error) {
            res.status(500).send({ message: String(process.env.STATUS_500) });
    ...
    APP.get(Routes.getCanditadoById, Middleware.authenticateTokenCand, async (req, res) => {
        console.log(`Requisição recebida em ${Routes.getCanditadoById}`);
        try {
            const id = String(req.params.id);
            console.log(`[GET ${Routes.getCanditadoById}] Requisição recebida, corpo:`, id);
            let [status, message] = await controllerCandidate.controllerGetCandidatoById(id);
            res.status(status).send({ message: message });
        }
        catch (error) {
            res.status(500).send({ message: String(process.env.STATUS_500) });
    ...
    APP.delete(Routes.deleteCanditado, Middleware.authenticateTokenCand, async (req, res) => {
        console.log(`Requisição recebida em ${Routes.deleteCanditado}`);
        try {
            const id = String(req.params.id);
            console.log(`[GET ${Routes.deleteCanditado}] Requisição recebida, corpo:`, id);
            let [status, message] = await controllerCandidate.controllerDeleteCandidato(id);
            res.status(status).send({ message: message });
        }
        catch (error) {
            res.status(500).send({ message: String(process.env.STATUS_500) });
    ...
    APP.put(Routes.updateCanditado, Middleware.authenticateTokenCand, async (req, res) => {
        const id = String(req.params.id);
        const body = req.body;
    ...
    APP.get(Routes.getContratante, Middleware.authenticateTokenEmp, async (req, res) => {
    ...
    APP.get(Routes.getContratanteById, Middleware.authenticateTokenEmp, async (req, res) => {
        const id = String(req.params.id);
    ...
    APP.delete(Routes.deleteContratante, Middleware.authenticateTokenEmp, async (req, res) => {
        const id = String(req.params.id);
    ...
    APP.put(Routes.updateContratante, Middleware.authenticateTokenEmp, async (req, res) => {
        const id = String(req.params.id);
        const body = req.body;
    ...
    // -----------------------------------
    APP.post(Routes.createColaborador, Middleware.authenticateTokenEmp, async (req, res) => {
        let body = req.body;
    ...
    APP.get(Routes.getColaborador, Middleware.authenticateTokenEmp, async (req, res) => {
        const id = String(req.params.id);
        console.log(`🚀 [GET /colaborador/${id}] Requisição recebida`);
        console.log(`Tipo do ID recebido: ${typeof id}`);
        let result = await controllerColaborador.controllerGetColaborador("tb_empresa", id);
        if (result) {
    ...
    APP.post(Routes.candidatarVaga, Middleware.authenticateTokenCand, (req, res) => {
    ...
    APP.get(Routes.getVagasById, Middleware.authenticateTokenEmp, async (req, res) => {
    ...
    APP.delete(Routes.deleteVaga, Middleware.authenticateTokenEmp, async (req, res) => {
        let id = String(req.params.id);
    ...
    // Rotas CRUD Evento
    // -----------------------------------
    APP.post(Routes.createEvento, Middleware.authenticateTokenEmp, async (req, res) => {
        const body = req.body;
    ...
    APP.delete(Routes.deleteEvento, Middleware.authenticateTokenEmp, async (req, res) => {
        const id = String(req.params.id);
        ...
        try {
            ...
            if (result && result.rowCount > 0) {
                ...
                console.warn(`⚠️ [DELETE /evento/${id}] Nenhum evento encontrado para deletar.`);
                res.status(404).json({
                    success: false,
            ...
            console.error(`💥 [DELETE /evento/${id}] Erro ao deletar evento:`, error);
            res.status(500).json({
                success: false,
    ...
    // -----------------------------------
    APP.post(Routes.createCalendario, Middleware.authenticateTokenEmp, async (req, res) => {
        const id = req.params.id;
        ...
        try {
            ...
            if (result) {
                ...
                console.warn(`⚠️ [POST /calendario/${id}] Falha ao criar calendário.`);
                res.status(400).json({
                    success: false,
            ...
            console.error(`💥 [POST /calendario/${id}] Erro ao criar calendário:`, error);
            res.status(500).json({
                success: false,
    ...
    APP.get(Routes.getCalendario, async (req, res) => {
        ...
        try {
            ...
            if (result && result.length > 0) {
                ...
                console.warn(`⚠️ [GET /calendario/${id}] Nenhum calendário encontrado.`);
                res.status(404).json({
                    success: false,
            ...
            console.error(`💥 [GET /calendario/${id}] Erro ao buscar calendários:`, error);
            res.status(500).json({
                success: false,
